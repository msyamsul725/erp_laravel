...
<head>
    ...
    <style>
        .sidebar-gradient {
            background: linear-gradient(180deg, #167786 10%, #167786 6%, #167786 6%);
        }
        ...
        .hover-glow:hover {
            box-shadow: 0 0 20px rgba(34, 164, 184, 1);
        }
        ...
        ::-webkit-scrollbar-thumb:hover {
            background: linear-gradient(45deg, #22A4B8, #6a4190);
        }
    </style>
    @vite([])
</head>
...
<body class="font-sans antialiased bg-gray-50">
    @auth
        <div class="min-h-screen">
            <!-- Sidebar -->
            <div id="sidebar"
                class="sidebar-toggle fixed inset-y-0 left-0 z-50 w-64 sidebar-gradient shadow-xl overflow-y-auto h-screen transform -translate-x-full md:translate-x-0 transition-transform duration-300 ease-in-out">
                ...
                <div class="flex items-center justify-center h-16 px-4 bg-black bg-opacity-20">
                    <div class="flex items-center">
                        ...
                        <span class="ml-3 text-white font-bold text-lg">{{ config('app.name', 'Attendance') }}</span>
                    </div>
                </div>
                ...
                <nav class="mt-8 px-4">
                    <div class="space-y-2">
                        ...
                        @include('components.sidebar.inventory')




                        @include('components.sidebar.user-management')
                        <!-- Role Management -->
                        ...
                        <!-- Account Settings -->
                        @include('components.sidebar.account')
                    </div>
                </nav>
                                ...
                                <nav>
                                    <ol class="flex items-center space-x-2 text-sm">
                                        ...
                                        @php
                                            ...
                                            // Hapus 'index' kalau di akhir
                                            if (end($parts) === 'index') {
                                                array_pop($parts);
                                            }

                                            // Hapus breadcrumb ganda Dashboard > Dashboard
                                            if (count($parts) === 1 && $parts[0] === 'dashboard') {
                                                $parts = [];
                                            }
                                            ...
                                            $labels = [
                                                ...
                                                'settings' => 'Settings',
                                                'receiving' => 'Receiving',
                                                'input-head-location' => 'Head Location',
                                                'input-location' => 'Location',
                                                'list-part-area' => 'Part Area',
                                                'manage-part-data' => 'Part Data',
                                                'report' => 'Report',
                                            ];

                                            $pretty = fn($seg) => $labels[$seg] ??
                                        ...
                                        @foreach ($parts as $i => $seg)
                                            @php
                                                ...
                                                if (!$isLast) {
                                                    if (Route::has($path)) {
                                                        $linkRoute = $path;
                                                    } elseif (Route::has($path . '.index')) {
                                                        $linkRoute = $path . '.index';
                                                    }
                                                }
                                            @endphp
                                            ...
                                            <li>
                                                <div class="flex items-center">
                                                    ...
                                                    @else
                                                        <span
                                                            class="ml-2 text-gray-700 font-medium">{{ $pretty($seg) }}</span>
                                                    @endif
                                                </div>
                                            </li>
                                        @endforeach
                            ...
                            <div class="flex items-center space-x-4">
                                ...
                                <button class="p-2 text-gray-400 hover:text-gray-600 relative">
                                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M15 17h5l-5 5-5-5h5v-8a4.5 4.5 0 00-4.5-4.5h-1"></path>
                                    </svg>
                            ...
                            <div class="flex items-center space-x-3 text-gray-600">
                                ...
                                <div>
                                    <div class="font-semibold text-gray-900">
                                        {{ config('app.name', 'Laravel Boilerplate') }}</div>
    ...
    <script>
        ...
        document.addEventListener('livewire:init', () => {
            ...
            Livewire.on('show-confirm', (event) => {
                ...
                console.log("📢 show-confirm dipanggil");
                console.log("Method yang mau dijalankan:", data.method);
                console.log("Params:", data.params);

                showConfirm(data.title, data.text, data.confirmText, data.cancelText)
                    .then((result) => {
                        if (result.isConfirmed && data.method) {
                            const comp = Livewire.find(data.componentId);
                            comp.call(data.method, ...(Object.values(data.params || {})));
                        } else {
