                ...
                <div class="btn-icon">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M6 7V6a2 2 0 012-2h8a2 2 0 012 2v1M6 7h12M6 7v10a2 2 0 002 2h8a2 2 0 002-2V7M9 11h6" />
                    </svg>
                </div>
                <div>
                    <h2 class="text-2xl font-bold text-gray-900">Head Location Management</h2>
                    <p class="text-sm text-gray-600 mt-1">Manage system head location</p>
                </div>
            </div>
            @permission('users.create')
                <button wire:click="$dispatch('openForm')" class="btn-primary">
        ...
        <div
            ...
            <div class="flex-1">
                <div class="relative">
                    ...
                    <input wire:model.live="search" type="text" placeholder="Search Head Location By Name..."
        ...
        <table class="min-w-full">
            <thead class="bg-gradient-to-r from-gray-50 to-gray-100">
                <tr>
                    ...
                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">Max Lantai</th>
                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">Max Rak</th>
                    {{-- Created At --}}
            ...
            <tbody class="bg-white divide-y divide-gray-100">
                @forelse($headLocations as $index =>  $loc)
                    <tr
                        class="hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50 transition-all duration-300 group">
                        <td class="px-6 py-5">{{ $headLocations->firstItem() + $index }}</td>

                        <td class="px-6 py-5 whitespace-nowrap">
                            <div class="flex items-center">
                                <div
                                    class="w-12 h-12 btn-icon rounded-xl flex items-center justify-center mr-4 shadow-md group-hover:shadow-lg transition-shadow duration-300">
                                    <span
                                        class="text-sm font-bold text-white">{{ substr($loc->location_name, 0, 2) }}</span>
                                </div>
                                <div>
                                    <div class="text-sm font-semibold text-gray-900">{{ $loc->location_name }}</div>
                                    <div class="text-xs text-gray-500">System head location</div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-5">{{ $loc->max_lantai }}</td>
                        <td class="px-6 py-5">{{ $loc->max_rak }}</td>
                        <td class="px-6 py-5 whitespace-nowrap">
                            <div class="flex items-center text-sm text-gray-500">
                                ...
                                <div>
                                    <div class="font-medium">{{ $loc->created_at->format('M d, Y') }}</div>
                                    <div class="text-xs">{{ $loc->created_at->diffForHumans() }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-5 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-2">
                                @permission('users.edit')
                                    <button wire:click="$dispatch('openForm', { headId: {{ $loc->id }} })"
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <h3 class="text-xl font-semibold text-gray-900 mb-2">No Head Location found</h3>
                                <p class="text-gray-500 mb-6 max-w-sm text-center">
                                    @if ($search)
                                        Try adjusting your search or filter criteria to find what you're looking
                                        for.
                                    @else
                                        Get started by creating your first Head Location.
                                    @endif
                                </p>
                                @if (!$search)
                                    @permission('users.create')
                                        <button wire:click="$dispatch('openForm')"
    ...
    <div class="px-6 py-4 bg-gradient-to-r from-gray-50 to-blue-50 border-t border-gray-200 rounded-b-2xl">
        <div class="flex items-center justify-between">
            <div class="text-sm text-gray-600">
                Showing <span class="font-medium">{{ $headLocations->firstItem() ?? 0 }}</span> to <span
                    class="font-medium">{{ $headLocations->lastItem() ?? 0 }}</span> of <span
                    class="font-medium">{{ $headLocations->total() }}</span> positions
            </div>
