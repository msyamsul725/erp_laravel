    ...
    <div class="bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 px-6 py-6 border-b border-gray-200">
        <div class="flex justify-between items-center">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h2 class="text-2xl font-bold text-gray-900">Job Titles Management</h2>
                    <p class="text-sm text-gray-600 mt-1">Manage system job titles and their access</p>
                </div>
            </div>
            @permission('users.create')
                <button wire:click="$dispatch('openPositionForm')" class="btn-primary">
        ...
        <div
            ...
            <div class="flex-1">
                <div class="relative">
                    ...
                    <input wire:model.live="search" type="text" placeholder="Search job titles by name, email..."
                        ...
                        <td class="px-6 py-5 whitespace-nowrap">
                            <div class="flex items-center">
                                <div
                                    ...
                                    <span
                                        class="text-sm font-bold text-white">{{ substr($position->title, 0, 2) }}</span>
                                </div>
                                <div>
                                    <div class="text-sm font-semibold text-gray-900">{{ $position->title }}</div>
                                    <div class="text-xs text-gray-500">System position</div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-5 whitespace-nowrap">
                            <div class="flex flex-wrap gap-2">
                                @if ($position->level)
                                    @php
                                        $levelStyles = [
                                            'junior' => 'from-green-100 to-green-200 text-green-800',
                                            'middle' => 'from-blue-100 to-blue-200 text-blue-800',
                                            'senior' => 'from-purple-100 to-purple-200 text-purple-800',
                                            'lead' => 'from-yellow-100 to-yellow-200 text-yellow-800',
                                        ];
                                        $style =
                                            $levelStyles[$position->level] ?? 'from-gray-100 to-gray-200 text-gray-800';
                                    @endphp

                                    <span
                                        class="inline-flex items-center px-3 py-1.5 rounded-xl text-xs font-semibold 
                bg-gradient-to-r {{ $style }} shadow-sm capitalize">
                                        <svg class="w-3 h-3 mr-1.5" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z">
                                ...
                                @else
                                    <span class="text-gray-400 italic">No Level</span>
                                @endif
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-5 whitespace-nowrap">
                            <span
                                ...
                                <div
                                    class="w-2 h-2 rounded-full mr-2 {{ $position->is_active ? 'bg-green-500 animate-pulse' : 'bg-red-500' }}">
                                </div>
                        ...
                        <td class="px-6 py-5 whitespace-nowrap">
                            <div class="flex items-center text-sm text-gray-500">
                                ...
                                <div>
                                    <div class="font-medium">{{ $position->created_at->format('M d, Y') }}</div>
                                    <div class="text-xs">{{ $position->created_at->diffForHumans() }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-5 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-2">
                                @permission('users.edit')
                                    <button
                                        wire:click="$dispatch('openPositionForm', { positionId: {{ $position->id }} })"
                        ...
                        <td colspan="5" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-gray-500 mb-6 max-w-sm text-center">
                                    ...
                                    @else
                                        Get started by creating your first position to manage system roles.
                                    @endif
                                </p>
                                @if (!$search)
                                    @permission('users.create')
                                        <button wire:click="$dispatch('openPositionForm')"
    ...
    <div class="px-6 py-4 bg-gradient-to-r from-gray-50 to-blue-50 border-t border-gray-200 rounded-b-2xl">
        <div class="flex items-center justify-between">
            <div class="text-sm text-gray-600">
                Showing <span class="font-medium">{{ $positions->firstItem() ?? 0 }}</span> to <span
                    class="font-medium">{{ $positions->lastItem() ?? 0 }}</span> of <span
                    class="font-medium">{{ $positions->total() }}</span> positions
            </div>
